...
const LoginPage = () => {
	...
	const handleGoogleAuth = async () => {
		...
		await supabase.auth.signInWithOAuth({
			provider: "google",
			options: {
				redirectTo: `${location.origin}/auth/callback?role=candidate`,
			},
		});
	};
			...
			if (authMode === "signup") {
				const { error } = await supabase.auth.signUp({
					...
					options: {
						data: {
							role: "candidate",
							name: data.name // Add name to user metadata
						},
						emailRedirectTo: `${location.origin}/auth/callback?role=candidate`,
					},
				});
				if (error) {
